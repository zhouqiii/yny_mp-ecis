                ...
                <ul class="infoList">
                  <li v-for="(o,i) in item.serious" :key="i" class="criticalBox">
                    <el-row>
                      ...
                      <el-col :span="9">正在就诊</el-col>
                      <el-col :span="5">{{o.doctorId}}</el-col>
                    </el-row>
                  </li>
                  <li v-for="(obj,i) in item.light" :key="i" :class="{level1:obj.level==3,level2:obj.level==4}">
                    <el-row>
                      ...
                      <el-col :span="9">正在就诊</el-col>
                      <el-col :span="5">{{obj.doctorId}}</el-col>
                    </el-row>
                  </li>
                </ul>
              </div>
...
export default class Screen extends Vue {
  ...
  public sendMessage(doctors: any[]) {
    ...
    this.stompClient.connect({}, (frame: any) => {
      ...
      this.stompClient.subscribe('/toAll/bulletScreenCalled',
        ({ body }: any) => {
          this.data = JSON.parse(JSON.parse(body).data).data;
          this.status = JSON.parse(JSON.parse(JSON.parse(body).data).status).status;
          // let firstPatient = {};
          // if (this.data.length === 3) {
          //   firstPatient = this.data[2];
          // }
            // if (!(firstPatient as any).firstItem) {
            //   console.log('没有叫号数据');
            //   return;
            // }
          ...
          //   }
          // }
          // (firstPatient as any) = doctorsList(doctors, firstPatient);
          this.data.splice(2, 1);
          this.data.forEach((item: { serious: any[], light: any[] }) => {
            // item.serious = item.serious.length <= 5 ? item.serious : item.serious.filter((o, i) => i < 5);
      ...
      this.stompClient.send('/chatCalled', {}, JSON.stringify({status: '0' }));
    });
  }
  public deptList(name?: any) {
    this.$store.dispatch('DeptList').then(({ data }) => {
      ...
      this.tabName[1] = data.data[0].children[0].name;
      this.tabName[0] = data.data[0].children[1].name;
    });
  }
  public playAudio() {
