...
export default class Bed extends Vue {
  ...
  public onGetRowData(row: any) {
    sessionStorage.setItem('beds', JSON.stringify(row));
  }
  /**
   * 刷新页面
   */
   ...
   */
  public onAddBeds() {
        this.$router.push({name: 'beds_add'});
  }
  /**
   * 编辑页面
   */
  public onEditBeds() {
        this.$router.push({name: 'beds_edit'});
  }
  /**
   * 删除页面
   */
  public onDelBeds() {
    if (sessionStorage.getItem('beds')) {
       const bed = JSON.parse((sessionStorage.getItem('beds') as string));
       this.$store.dispatch('DelBed', bed.id).then(({data}) => {
        if (data.code === 0) {
          sessionStorage.setItem('beds', '');
          this.onGetBedList();
