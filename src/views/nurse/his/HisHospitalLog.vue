<template>
  <div class="HisHospitalLog">
    <el-row>
      <el-col :span="18">
        <el-tabs v-model="activeName">
          <el-tab-pane label="住院记录" name="hospitalPatientVisit" :disabled="isHospitalPatientVisitDataEmpty">
						<HisHospitalPatientVisit :rowData="rowData"></HisHospitalPatientVisit>
					</el-tab-pane>
          <el-tab-pane label="住院医嘱" name="hospitalPatientDoctorOrder" :disabled="isRowDataEmpty">
						<HisHospitalPatientDoctorOrder v-if="activeName==='hospitalPatientDoctorOrder'" :rowData="rowData"></HisHospitalPatientDoctorOrder>
					</el-tab-pane>
      ...
      <el-col :span="6" class="historyList">
        ...
        <el-table :data="tableData" border style="width: 100%" :height="tableHeight" highlight-current-row
          ...
          <el-table-column prop="admissionDateTime" label="入院时间"/>
          <el-table-column prop="dischargeDateTime" label="出院时间"/>
        </el-table>
      </el-col>
    </el-row>
  </div>
...
import HisHospitalPatientDoctorOrder from '@/components/nurse/his/HisHospitalPatientDoctorOrder.vue';
import HisHospitalPatientVisit from '@/components/nurse/his/HisHospitalPatientVisit.vue';
...
export default class HisHospitalLog extends Vue {
  // private $!: any;
  private activeName = 'hospitalPatientVisit';
  ...
  private handleCurrentChange(val: any) {
    this.rowData = val;
    this.activeName = 'hospitalPatientVisit';
  ...
  private async getData(patientId: string) {
      ...
      const resData = await this.$store.dispatch('HisHospitalPatientVisitGet', {patientId, ...this.form});
  ...
  private watchPatient(newVal: any) {
    this.getData(newVal.hisCode).then(() => this.activeName = 'hospitalPatientVisit');
    // this.getData('R38893').then(() => this.activeName = 'hospitalPatientVisit');
  }
  private get isPatientAnalysisRowDataEmpty() {
    return $.isEmptyObject(this.patientAnalysisRowData);
  }
  private get isHospitalPatientVisitDataEmpty() {
